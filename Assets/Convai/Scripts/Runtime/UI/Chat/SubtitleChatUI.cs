    ...
    public class SubtitleChatUI : ChatUIBase
    {
        ...
        private void UpdateSubtitleText(string speakerName, string text, Color color)
        {
            ...
            if (_subtitle != null)
            {
                ...
                _subtitle.MessageTextObject.text = text;//Nội dung tin nhắn (Áp dụng cho CẢ Player và NPC)

                // --- BẮT BUỘC MÀU ĐEN VÀ CỠ 32 ---
                _subtitle.MessageTextObject.color = Color.black;              
                _subtitle.MessageTextObject.fontSize = 32;               
                _subtitle.RTLUpdate();
            }
            else
            {
